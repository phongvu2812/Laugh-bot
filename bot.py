...
async def connect_nodes():
    ...
    await wavelink.NodePool.create_node(
        bot=client,
        host='nhập ip của máy bạn ở đây',
        port=2333,
...
async def play(ctx, *, search: wavelink.YouTubeTrack):
    ...
    if vc.is_playing():
        ...
        await ctx.send(embed=discord.Embed(
            title=search.title,
            url=search.uri,
            description=f'Queued {search.title} ordered by {ctx.author}'))
    else:
        await vc.play(search)
        await ctx.send(embed=discord.Embed(
            title=search.title,
            url=search.uri,
            description=f'Playing {vc.source.title} ordered by {ctx.author}'))


@client.command()
async def p(ctx, *, search: wavelink.YouTubeTrack):
    vc = ctx.voice_client
    if not vc:
        custom_player = CustomPlayer()
        vc: CustomPlayer = await ctx.author.voice.channel.connect(cls=custom_player)
